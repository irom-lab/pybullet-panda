...
class GraspEnv(PandaEnv):
    ...
    def init_joint_angles(self):
        ...
        """
        return [
            0, -0.255, 0, -2.437, 0, 2.181, 0.785, 0, 0, self._finger_open_pos,
            0.00, self._finger_open_pos, 0.00
        ]
    ...
    def reset_task(self, task=None):
        ...
        obj_id = self._p.loadURDF(
            task.obj_path, basePosition=task.obj_pos,
            baseOrientation=task.obj_quat, globalScaling=task.global_scaling
        )
    ...
    def step(self, action):
        ...
        self.reset_arm_joints_ik(
            self.initial_ee_pos_before_img, self.initial_ee_orn
        )
        ...
        ee_pos[2] = np.maximum(
            0, ee_pos[2] + self.ee_finger_offset + self._grasp_z_offset
        )
        ...
        self.move_pose(
            ...
            num_steps=100,
        )  # keep pose until gripper closes
        self.move_pose(
            ee_pos_after,
            absolute_global_quat=ee_orn,
            num_steps=150,
        ...
        return np.array([]), reward, True, {
            'global_scaling': self.task['global_scaling']
        }  # s, reward, done, info

...
class SweepEnv(PandaEnv):
    ...
    def reset_task(self, task):
        ...
        self.initial_dist = self._get_min_dist_from_finger(
            self.tool_id, max_dist=self._obj_max_dist
        )
    ...
    def step(self, action):
        ...
        if self.grasp_executed and self._get_lowerest_pos(
        )[2] > self._regrasp_threshold and not self._check_hold_object(
            self.tool_id
        ):
            self.grasp_executed = False
        ...
        # Check cylinder
        cylinder_pos, _ = self._p.getBasePositionAndOrientation(
            self.cylinder_id
        )

        # Reward - approaching tool
        dist = self._get_min_dist_from_finger(
            self.tool_id, max_dist=self._obj_max_dist
        )
        ...
        if self._check_hold_object(self.tool_id):
            ...
            cylinder_dist = self._get_min_dist_between_obj(
                self.tool_id, self.cylinder_id,
            ...
            if self._get_min_dist_between_obj(
                self.cylinder_id, self._panda_id,
                max_dist=self._cylinder_contact_margin
            ) >= self._cylinder_contact_margin:

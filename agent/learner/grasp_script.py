...
class GraspScript():
    ...
    def __init__(self, cfg):
        ...
        self.thetas = torch.from_numpy(
            np.linspace(0, 1, num=cfg.num_theta, endpoint=False) * np.pi
        )
    ...
    def forward(self, obs, extra=None, **kwargs):
        ...
        for n in range(N):
            ...
            # perturb
            py_single += self.rng.integers(
                -self.pixel_perturb, self.pixel_perturb + 1, size=1
            )
            px_single += self.rng.integers(
                -self.pixel_perturb, self.pixel_perturb + 1, size=1
            )
        ...
        xy_all = np.concatenate(
            (self.p2x[px_all][None], self.p2y[py_all][None])
        ).T
        ...
        )  # unnormalize (min_z corresponds to max height and max_z corresponds to min height)
        ...
        rot_all = np.concatenate((
            np.concatenate((
                np.cos(theta_all)[:, None,
                                  None], -np.sin(theta_all)[:, None, None]
            ), axis=-1),
            np.concatenate((
                np.sin(theta_all)[:, None, None], np.cos(theta_all)[:, None,
                                                                    None]
            ), axis=-1)
        ), axis=1)
        py_offset_all = py_all - (H-1) / 2
        px_offset_all = px_all - (W-1) / 2
        pxy_offset_all = np.hstack(
            (px_offset_all[:, None], py_offset_all[:, None])
        )
        pxy_rot_offset_all = np.matmul(rot_all, pxy_offset_all[:, :,
                                                               None])[:, :, 0]

...
class AgentImitate(AgentBase):
    ...
    def load_data(self, path):
        """Imitation data. Assume all reward=1"""
        data = load_obj(path)
        self.depth_buffer = data['depth'].float().to('cpu')
        action_all = data['action']
        
        # Construnct ground truth and mask (all zeros except for selected pixel)
        self.ground_truth_buffer = torch.zeros_like(self.depth_buffer).to('cpu')
        self.mask_buffer = torch.zeros_like(self.depth_buffer).to('cpu')

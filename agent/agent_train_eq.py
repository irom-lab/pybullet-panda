...
class AgentTrainEq(AgentTrain):
    ...
    def learn(
        ...
        while self.cnt_step <= self.max_sample_steps:
            ...
            if not self.eval_mode:
                ...
                for update_ind in range(self.num_update):
                    if flag_train_action_decoder:
                        ...
                        action_decoder_ce_loss = self.learner.update_action_decoder(
                    ...
                    batch_train = self.unpack_batch(
                        self.sample_batch(
                            positive_ratio=self.batch_positive_ratio
                        )
                    )
                    latent_policy_ce_loss, alignment_loss, stats = self.learner.update_latent_policy(
                        batch_train, verbose=update_ind < 1
                    )
                    loss = np.vstack((
                        loss, [
            ...
            # Evaluate
                ...
                if verbose:
                    ...
                    logging.info(
                        f'Avg cumulative reward: {eval_reward_cum_avg}'
                    )
                    logging.info(
                ...
                logging.info(
                    f'Training action decoder next? {flag_train_action_decoder}'
                )
                ...
                #!
                self.eps_schduler = StepLRFixed(
                    initValue=1 - eval_reward_cum_avg, period=eps_period,
                    endValue=self.cfg_eps.end, stepSize=self.cfg_eps.step
                )
        ...
        logging.info(
            'Saving best path {} with reward {}!'.format(
    ...
    def sample_batch(self, batch_size=None, positive_ratio=None):
        ...
        return (
            depth_batch, ground_truth_batch, mask_batch, action_batch,
            reward_batch, scaling_batch
        )
    ...
    def store_transition(self, s, a, r, s_, done, info):
        ...
        new_depth = rotate_tensor(new_depth,
                                  theta=torch.tensor(theta)).squeeze(1)
        ...
        if self.depth_buffer.shape[0] < self.memory_capacity:
            self.depth_buffer = torch.cat((self.depth_buffer, new_depth)
            ...
            self.action_buffer = torch.cat((self.action_buffer, action_tensor)
                                          )[:self.memory_capacity]
            self.reward_buffer = torch.cat((self.reward_buffer, reward_tensor)
                                          )[:self.memory_capacity]
            self.scaling_buffer = torch.cat(
                (self.scaling_buffer, scaling_tensor)
            )[:self.memory_capacity]
        else:
            # Replace older ones
    ...
    def reset_memory(self, memory):
        ...
        self.action_buffer = torch.empty((0, self.action_dim)
                                        ).float().to('cpu')
    ...
    def save_sample_affordance(self, num):
        for ind in range(num):
            ...
            img_input = img[np.newaxis,
                            np.newaxis].float().to(self.device)  # 1x1xHxW
            pred = self.learner(img_input).squeeze(1).squeeze(0)  # HxW
            img_path_prefix = os.path.join(
                self.img_folder,
                f'step-{str(self.cnt_step)}_scaling-{scaling:.3f}'
            )

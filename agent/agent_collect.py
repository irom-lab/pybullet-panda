...
class AgentCollect(AgentTrain):
    ...
    def __init__(self, cfg, venv, verbose=True):
        ...
        self.eps_schduler = StepLRFixed(
            initValue=self.cfg_eps.init, period=1000,
    ...
    def learn(
        ...
        for batch_ind in range(num_batch):
            ...
            logging.info(
                f'Batch {batch_ind+1} out of {num_batch}, number of positive grasps in the buffer: {len(torch.where(self.reward_buffer == 1)[0])}'
            )
    ...
    def store_transition(self, s, a, r, s_, done, info):
        ...
        self.obs_buffer = torch.cat((self.obs_buffer, (obs_rot * 255).byte())

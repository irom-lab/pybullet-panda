...
class GraspSim:
    ...
    def update_policy(self, model_dict):
        self.fcn.load_state_dict(model_dict)
    ...
    def sim_step(self,
        ...
        success_trials = []
        depth_rot_trials = np.empty((0, self.img_size, self.img_size))
        ...
        for obj_path_list, obj_height_list, use_random_object in zip(
            ...
            for use_random in use_random_trial:
                ...
                # Infer
                depth_orig = torch.from_numpy(
                    self.get_depth()[np.newaxis, np.newaxis]).to('cpu')
                depth_rot_all = torch.empty(
                    (0, 1, self.img_size, self.img_size))
                for theta in self.thetas:
                    depth_rotated = rotate_tensor(depth_orig, theta=theta)
                    depth_rot_all = torch.cat((depth_rot_all, depth_rotated))
                # plt.imshow(depth_orig[0,0], cmap='Greys', interpolation='nearest')
                # plt.show()
                # for depth in depth_rot_all:
                # 	plt.imshow(depth[0], cmap='Greys', interpolation='nearest')
                # 	plt.show()
                pred_infer = self.fcn(depth_rot_all).squeeze(
                    1).detach().numpy()  # num_theta x img_size x img_size
                ...
                if not use_random:
                    (theta_ind, px,
                     py) = np.unravel_index(np.argmax(pred_infer),
                                            pred_infer.shape)
                else:
                    theta_ind = random.randint(0, self.num_theta - 1)
                ...
                if not (
                    ...
                    depth_rot_trials = np.concatenate(
                        (depth_rot_trials, depth_rot_all[theta_ind,
                                                         0][np.newaxis]))
        ...
        p.disconnect()
        return success_det_trials, success_trials, depth_rot_trials, pred_trials
    ...
    def clear_obj(self):
        ...
        for obj_id in self.obj_id_list:
            ...
            # hold_object = self.check_hold_object(objId)
            # if hold_object:
            # p.removeBody(objId) # remove obj from env
            # self.objId_list.remove(objId)
            # return objId
...
class TrainGrasp:
    def __init__(
        ...
        self.graspSim = GraspSim(mu=mu,
                                 sigma=sigma,
                                 img_size=img_size,
                                 num_theta=num_theta,
                                 inner_channels=inner_channels,
                                 delta_z=delta_z,
                                 max_obj_height=max_obj_height)

        # Set up model
        self.fcn = FCN(inner_channels=self.inner_channels,
                       out_channels=1,
                       img_size=self.img_size).to(device)
        num_model_parameter = sum(p.numel() for p in self.fcn.parameters()
                                  if p.requires_grad)
        ...
        for step in range(num_step):
            ...
            if step % affordance_map_freq == 0:
                ...
                depth_input = torch.from_numpy(
                    new_depth[depth_ind][np.newaxis, np.newaxis]).float().to(
                        self.device)  # 1x1xHxW
                pred_infer = self.fcn(depth_input).squeeze(1).squeeze(0)  # HxW
    ...
    def train_policy(self):
        ...
        sample_inds = random.sample(range(self.buffer_size), k=self.N)
    ...
    def save_model(self, path):
        torch.save(self.fcn.state_dict(), path + '.pt')

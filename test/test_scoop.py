...
for _ in range(10):
    ...
    obs_id = p.createMultiBody(
        baseMass=0.002, baseCollisionShapeIndex=obs_collision_id,
        baseVisualShapeIndex=obs_visual_id, basePosition=[
            0.6 + random.random() * 0.003, 0.1 + random.random() * 0.003, 0.005
...
env.move_pos([0.42, 0.1, 0.195],
             absolute_global_euler=[-np.pi, np.pi + np.pi / 20,
                                    0], numSteps=500)
# while 1:
#     continue
...
for i in range(10):
    ...
    if i > 5:
        z_vel = -(i - 5) * 0.1
    env.velocity_control([x_vel, 0.0, 0.0], [0, z_vel, 0], numSteps=5,
                         objId=spatula_id)
    # time.sleep(0.1)
...
for i in range(10):
    x_vel -= 0.1
    z_vel += 0.005
    env.velocity_control([x_vel, 0.0, 0.01], [0, z_vel, 0], numSteps=5,
                         objId=spatula_id)
    # time.sleep(0.1)

# Level
env.velocity_control([0, 0.0, 0.01], [0, 0.01, 0], numSteps=20,
                     objId=spatula_id)
# time.sleep(0.1)

# Keep pose
env.velocity_control([0, 0.0, 0.005], [0.0, 0, 0], numSteps=2000,
                     objId=spatula_id)

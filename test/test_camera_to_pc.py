...
plane = p.loadURDF("plane100.urdf", globalScaling=1)
cube = p.loadURDF("cube.urdf", [0.5, 0, 0.1], globalScaling=0.1)

p.resetDebugVisualizerCamera(0.5, 180, -89.99, [0.50, 0.0, 0.0])
...
def getRayFromTo(mouseX, mouseY):
    ...
    lenFwd = math.sqrt(
        rayForward[0] * rayForward[0] + rayForward[1] * rayForward[1]
    ...
    lenOrtho = math.sqrt(
        ortho[0] * ortho[0] + ortho[1] * ortho[1] + ortho[2] * ortho[2]
    )
...
for w in range(0, imgW, stepX):
    for h in range(0, imgH, stepY):
        ...
        rayFrom, rayTo, alpha = getRayFromTo(
            w * (width/imgW), h * (height/imgH)
        )
        rf = np.array(rayFrom)
        rt = np.array(rayTo)
        vec = rt - rf
        ...
        mb = p.createMultiBody(
            baseMass=0, baseCollisionShapeIndex=collisionShapeId,
